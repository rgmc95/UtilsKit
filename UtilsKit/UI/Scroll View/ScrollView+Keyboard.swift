...
extension UIScrollView {
    ...
    public func scrollWithKeyboard(_ value: Bool = true) {
        if value {
            NotificationCenter.default.addObserver(self, selector: #selector(keyboardAction), name: UIResponder.keyboardWillShowNotification, object: nil)
    ...
    @objc private func keyboardAction(notification: NSNotification) {
        ...
        if #available(iOS 11.0, *) {
            ...
            let top = UIApplication._shared?.topViewController?.navigationController?.isNavigationBarHidden ?? true ? 0 : UIApplication._shared?.topViewController?.navigationController?.navigationBar.frame.size.height ?? 0
